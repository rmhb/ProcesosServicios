...
public class GeneradorPrimos extends SwingWorker<Integer,Integer> {
    ...
    public GeneradorPrimos(JTextArea intermediateJTextArea, JButton getPrimesButton, JButton cancelButton, JLabel status, int max) {
        ...
        Arrays.fill(primos, true); // Inicializar todo a true, para implementar la criba de Eratosthenes para encontrar todos los primos menores que max. 
        // Lo que hacía era buscar el primer primo y tacha todos los múltiplos de dicho primo. Después busca el siguiente primo y vuelvee a tachar todos sus múltiplos y así sucesivamente. 
    }
    
    public Integer doInBackGround(){
        int count = 0; // Numero primos encontrados
        for (int i = 2; i < primos.length; i++) {
            if(isCancelled())// Si Cancelamos la busqueda
                return count;
            else{
